...
export function AscOrDescFilter({
  setOpenModal,
  enviarDatos,
  // enviarDatosPlaca,
  // enviarDatosMarca,
...
export function FilterModal({ setOpenModal, enviarDatos }) {
  const [filterOption, setFilterOption] = useState(0);
  const handleModalInputChange = () => {
    const attribute = sortAttributes[Number(filterOption)];
    if (!attribute) {
  ...
  return (
    <div className="fixed z-10 inset-0 flex items-center justify-center bg-opacity-200 ">
      <div className="w-full max-w-2xl  rounded-lg p-6 border border-gray-100 bg-gray-900/60">
        ...
        <div className="flex flex-col p-2 items-center justify-center">
          <h1 className="m-3">Seleccione una Opcion para filtrar</h1>
          <h4>1.CodAuto</h4>
          <h4>2.Placa</h4>
          <h4>3.Marca</h4>
          <h4>4.Tipo</h4>
